...
FOLDER_IN_ARCHIVE_THREE_SEC_REPR_AVG = "latent_small_avg"
...
my_train_data = SV_LIBRISPEECH('/home/Daniel/DeepProject/',
                               folder_in_archive = FOLDER_IN_ARCHIVE_ORIGINAL_LIBRI, download=True,file_ext='.flac',url = "test-clean")
...
my_train_loader = torch.utils.data.DataLoader(my_train_data, batch_size=batch_size, shuffle=True,num_workers=4)
...
for epoch in range(epochs):
  ...
  for i, data in enumerate(my_train_loader, 0):
       with torch.no_grad():
        ...
        # print(f'Network thinks speaker is = {top_class} while true speaker is = {labels}')
