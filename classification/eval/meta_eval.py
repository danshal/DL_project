...
def meta_test(net, testloader, use_logit=True, is_norm=True, classifier='LR', opt=None):
    ...
    with torch.no_grad():
        for idx, data in enumerate(testloader):
            ...
            batch_size, _, height, width = support_xs.size()
            support_xs = support_xs.view(-1, height, width)
            query_xs = query_xs.view(-1, height, width)
            support_features = net(support_xs)
            query_features = net(query_xs)
            # support_features = net.feature_aggregator(net.feature_extractor(torch.squeeze(support_xs))).view(support_xs.size(0), -1)
            # query_features = net.feature_aggregator(net.feature_extractor(torch.squeeze(query_xs))).view(query_xs.size(0), -1)
            # if use_logit:
            ...
            if idx%2==1:
                print(f'finished {idx} batches.  current acc : { mean_confidence_interval(acc)} ')
                print(f'current acc : { mean_confidence_interval(acc)}')
